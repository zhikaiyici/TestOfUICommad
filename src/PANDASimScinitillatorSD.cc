...
PANDASimScinitillatorSD::PANDASimScinitillatorSD(
 ...
 : G4VSensitiveDetector(name), 
   //fRunAction(nullptr), fEventAction(nullptr), fStackManager(nullptr), fTrackingAction(nullptr),
    fHitsCollection(nullptr), fHitsNum(nofHits)
{
...
G4bool PANDASimScinitillatorSD::ProcessHits(G4Step* step, G4TouchableHistory*)
{
    ...
    G4int moduleNumber = (G4int)(fHitsNum * G4UniformRand());
    ...
    //if (!fEventAction)
    //    fEventAction = static_cast<B1::EventAction*>(G4EventManager::GetEventManager()->GetUserEventAction());
